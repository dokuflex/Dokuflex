            ...
            try
            {
                using (var process = new Process())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(errorMessage))
                        IO.Log(new Exception(string.Format("ExecuteShellCommand Error: {0}", errorMessage)));
                }
            }
            catch (Win32Exception _Win32Exception)
            {
                IO.Log(new Exception(string.Format("Win32 Exception caught in process: {0}", _Win32Exception.ToString())));
            }

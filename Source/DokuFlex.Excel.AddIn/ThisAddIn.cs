...
namespace DokuFlex.Excel.AddIn
{
    ...
    public partial class ThisAddIn
    {
        ...
        private void WorkbookOpen(Excel.Workbook Wb)
        {
            ...
            var trackingItem = TrackingListManager.GetByPath(Wb.FullName);
            ...
            if (trackingItem.ModifiedTime < documentMetadata.dateModified)
            {
                if (MessageBox.Show(String.Format("Existe una versión más reciente de {0} en DokuFlex.\n\n{1}",
                    Wb.Name.ToUpperInvariant(), "¿Desea actualizar el libro de cálculo con la versión de DokuFlex?"),
                    Globals.ThisAddIn.Application.Name, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    var currentPath = Wb.FullName;
                    ...
                    using (var form = new TransferProgressForm())
                    {
                        ...
                        {
                            TrackOff();
                            Globals.ThisAddIn.Application.Workbooks.Open(currentPath);
                            TrackOn();
        ...
        public void ShowMetadata()
        {
            ...
            var trackingItem = TrackingListManager.GetByPath(Globals.ThisAddIn.Application.ActiveWorkbook.FullName);
        ...
        {
            var path = Globals.ThisAddIn.Application.ActiveWorkbook.FullName;
            ...
            if (trackingItem == null)
            {
                MessageBox.Show(String.Format("El libro de cálculo no se puede añadir a los favoritos porque no existe en DokuFlex.\n\n{0}",
                    "Guarde el libro de cálculo en DokuFlex y repita la acción"),
                    this.Application.Name, MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
            }
            else
        ...
        public void SaveWorkbook()
        {
            ...
            if (String.IsNullOrWhiteSpace(this.Application.ActiveWorkbook.Path))
            {
                if (MessageBox.Show(String.Format("El libro de cálculo aún no ha sido guardado en disco.\n\n{0}",
                    "¿Desea guardar el libro de cálculo en su equipo para continuar?"),
                    ...
                    MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(this.Application.ActiveWorkbook.Path)) return;
                }
            }
            else
            {
                if (!this.Application.ActiveWorkbook.Saved &&
                    MessageBox.Show(String.Format("¿Desea guardar los cambios en {0} antes continuar?",
                    this.Application.ActiveWorkbook.Name.ToUpperInvariant()),
                    this.Application.Name, MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    this.Application.ActiveWorkbook.Save();
            ...
            var fileId = UploadFile(ticket, this.Application.ActiveWorkbook.FullName);

            if (newFile && !String.IsNullOrWhiteSpace(fileId))
            {
                using (var form = new MetadataForm(ticket, fileId, this.Application.ActiveWorkbook.Name))
                {
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        DokuFlexService.UpdateDocumentMetadata(ticket, form.DocumentType,
        ...
        public void OpenRecentWorkbook(string ticket, IDocument document)
        {
            TrackingListManager.Reload();
            ...
            if (trackingItem != null)
            {
                if (trackingItem.ModifiedTime < document.modifiedTime)
                {
                    using (var form = new TransferProgressForm())
                    {
                        ...
                        {
                            TrackOff();
                            Globals.ThisAddIn.Application.Workbooks.Open(trackingItem.Path);
                            TrackOn();
                ...
                else
                {
                    //Open the document.
                    TrackOff();
                    this.Application.Workbooks.Open(trackingItem.Path);
                    TrackOn();
                }
            }
            else
            {
                ...
                var currentDir = DFEnvironment.GetSpecialFolder(DFEnvironment.SpecialFolder.SpreadSheet);
